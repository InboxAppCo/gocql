...
func TestKeyspaceMetadata(t *testing.T) {
	...
	if len(tableMetadata.ClusteringColumns) != 1 {
		t.Errorf("expected clustering columns length of 1, but was %d", len(tableMetadata.ClusteringColumns))
	}
	for i, column := range tableMetadata.ClusteringColumns {
		if column == nil {
			t.Errorf("clustering column metadata at index %d was nil", i)
		}
	}
...
func TestTokenAwareConnPool(t *testing.T) {
	...
	// insertTrace := &bytes.Buffer{}
	query := session.Query("INSERT INTO test_token_aware (id, data) VALUES (?,?)", 42, "8 * 6 =")
	// query.Trace(NewTraceWriter(session, insertTrace))
	if err := query.Exec(); err != nil {
		t.Fatalf("failed to insert with err: %v", err)
	}
	// fmt.Printf("trace: %s", string(insertTrace.Bytes()))
	// selectTrace := &bytes.Buffer{}
	query = session.Query("SELECT data FROM test_token_aware where id = ?", 42).Consistency(One)
	// query.Trace(NewTraceWriter(session, selectTrace))
	iter := query.Iter()
	// fmt.Printf("trace: %s", string(selectTrace.Bytes()))
	var data string
	if !iter.Scan(&data) {
